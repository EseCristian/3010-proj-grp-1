...
def calculate_fte(prefix, ch, enrollment):
...
if tab == "faculty":
    ...
    for faculty in faculty_records:
	    ...
	    cursor.execute(""" SELECT cs.prefix, cs.enrollment, c.ch, c.gu FROM course_schedule AS cs JOIN courses AS c ON cs.prefix = c.prefix AND cs.number = c.number WHERE cs.year = %s AND cs.semester = %s AND cs.instructor = %s; """, (year, semester, id))
